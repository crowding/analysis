function trial = motion_energy_calc(trial, mkfig)

r = trial.eccentricity;
nt = trial.target_number_all;
...
%configure the single motion patch
CP = CauchyPatch...
    ( 'size', [trial.wavelengthScalar*r ...
               trial.widthScalar*r ...
               trial.durationScalar*trial.dt]...
    , 'order', trial.order ...
    , 'velocity', trial.wavelengthScalar*r*trial.tf ...
    , 'phase', 0 ... %a test... 
    );

%then the apparent motion based on it
motion = ApparentMotion...
    ( 'primitive', CP ...
    , 'dx', trial.abs_displacement ...
    , 'dt', trial.dt ...
    , 'n', trial.n + 1 ... % check that this does n + 1
    , 'center', [0 0 0] ...
    );

%then replicate the apparent motion around the circle (which is
%another kind of apparent motion, with zero offset.
if isnan(trial.target_number_shown) || trial.target_number_shown <= 0
    nt = trial.target_number_all;
else
    nt = trial.target_number_shown;
end
stim = ApparentMotion ...
    ( 'primitive', motion ...
    , 'dx', 2*pi*r / trial.target_number_all ...
    , 'dt', 0 ...
    , 'n', nt);

%we also need one with the direction of motion going the other way.
stim2 = stim;
stim2.primitive.primitive.velocity = -stim2.primitive.primitive.velocity;

%we will deal with the direction content later.

%now let's render a picture. We render it for an extent of 2*pi*r in
%the X-direction, and only at 0 in the y-direction. That it, we render
%the transect of the elements on the circle passing through their
%centroids.
...
%evaluate the motion.
z1 = evaluate(stim, xs, ys, ts);
z2 = evaluate(stim2, xs, ys, ts);
z = z1 .* trial.content_cw + z2 .* trial.content_ccw;
...
while size(z, 2) > np
    %this likely breaks on wrapping more than once but that never happens
    %in the experiment
...
%this should just be the same up to a factor (norms are preserved
%under orthonormal change of basis), but I'll double check
...
if (mkfig)
    subplot(3,2,4);
    barh(1./(r*wmults(:,1)), energies(:,1) / nelem / trial.wavelengthScalar / trial.durationScalar / trial.dt);
    hold on;
    barh(1./(r*wmults(:,2)), -energies(:,2) / nelem / trial.wavelengthScalar / trial.durationScalar / trial.dt);
    hold off;
...
trial.energy_cw = energies(:,1)';
trial.energy_ccw = energies(:,2)';
